module Webui::MaintenanceIncidentHelper
  ...
  def packages_cell(incident)
    packages = incident.packages.joins(:flags).where(flags: { flag: :build, status: 'enable', repo: incident.release_targets.
                                              select(:name) }).limit(2).pluck(:name)
  ...
  def release_targets_cell(incident)
    safe_join(
      [
        incident.release_target_repos_and_projects.map do |target_repo|
          content_tag(:div) do
            safe_join(
              [
                ...
                link_to(target_repo.prj_name, project_show_path(project: target_repo.prj_name))
              ]
            )
          end
        end
