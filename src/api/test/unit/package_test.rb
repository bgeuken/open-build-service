...
class PackageTest < ActiveSupport::TestCase
  ...
  def test_flags_to_axml
    # check precondition
    assert_equal 2, @package.flags.flags_of_type('build').size
    assert_equal 1, @package.flags.flags_of_type('publish').size
    assert_equal 1, @package.flags.flags_of_type('debuginfo').size
  ...
  def test_add_new_flags_from_xml
    ...
    # check results
    assert_equal 1, @package.flags.flags_of_type('build').size
    assert_equal 'enable', @package.flags.flags_of_type('build')[0].status
    assert_equal '10.2', @package.flags.flags_of_type('build')[0].repo
    assert_equal 'i586', @package.flags.flags_of_type('build')[0].architecture.name
    assert_equal 1, @package.flags.flags_of_type('build')[0].position
    assert_nil @package.flags.flags_of_type('build')[0].project
    assert_equal 'TestPack', @package.flags.flags_of_type('build')[0].package.name
    assert_equal true, @package.enabled_for?('build', '10.2', 'i586')
    assert_equal false, @package.disabled_for?('build', '10.2', 'i586')

    assert_equal 1, @package.flags.flags_of_type('publish').size
    assert_equal 'enable', @package.flags.flags_of_type('publish')[0].status
    assert_equal '10.1', @package.flags.flags_of_type('publish')[0].repo
    assert_equal 'x86_64', @package.flags.flags_of_type('publish')[0].architecture.name
    assert_equal 2, @package.flags.flags_of_type('publish')[0].position
    assert_nil @package.flags.flags_of_type('publish')[0].project
    assert_equal 'TestPack', @package.flags.flags_of_type('publish')[0].package.name

    assert_equal 1, @package.flags.flags_of_type('debuginfo').size
    assert_equal 'disable', @package.flags.flags_of_type('debuginfo')[0].status
    assert_equal '10.0', @package.flags.flags_of_type('debuginfo')[0].repo
    assert_equal 'i586', @package.flags.flags_of_type('debuginfo')[0].architecture.name
    assert_equal 3, @package.flags.flags_of_type('debuginfo')[0].position
    assert_nil @package.flags.flags_of_type('debuginfo')[0].project
    assert_equal 'TestPack', @package.flags.flags_of_type('debuginfo')[0].package.name
  end
  ...
  def test_delete_flags_through_xml
    ...
    assert_equal 0, @package.flags.flags_of_type('build').size
    assert_equal 0, @package.flags.flags_of_type('publish').size
  end
