...
class PublishFlagTest < ActiveSupport::TestCase
  ...
  def test_add_publish_flag_to_project
    ...
    # check the result
    assert_equal 4, @project.flags.flags_of_type('publish').size

    f = @project.flags.flags_of_type('publish')[2]
    ...
    f = @project.flags.flags_of_type('publish')[3]
    assert_kind_of Flag, f
  ...
  def test_add_publish_flag_to_package
    ...
    # check the result
    assert_equal 3, @package.flags.flags_of_type('publish').size

    f = @package.flags.flags_of_type('publish')[1]
    assert_kind_of Flag, f
    ...
    f = @package.flags.flags_of_type('publish')[2]
    assert_kind_of Flag, f
  ...
  def test_delete_type_publish_flags_from_project
    ...
    # destroy flags
    @project.flags.flags_of_type('publish')[1].destroy
    # reload required!
    @project.reload
    assert_equal 1, @project.flags.flags_of_type('publish').size
    assert_equal 1, count - Flag.all.size

    @project.flags.flags_of_type('publish')[0].destroy
    # reload required
    @project.reload
  ...
  def test_delete_type_publish_from_package
    ...
    # destroy flags
    @package.flags.flags_of_type('publish')[0].destroy
    # reload required!
    @package.reload
  ...
  def test_position
    ...
    @project.reload
    assert_equal 3, @project.flags.flags_of_type('publish').size
