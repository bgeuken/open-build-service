...
class BuildFlagTest < ActiveSupport::TestCase
  ...
  def test_add_build_flag_to_project
    ...
    # check the result
    assert_equal 4, @project.flags.flags_of_type('build').size

    f = @project.flags.flags_of_type('build')[2]
    ...
    f = @project.flags.flags_of_type('build')[3]
  ...
  def test_add_build_flag_to_package
    ...
    # check the result
    assert_equal 4, @package.flags.flags_of_type('build').size

    f = @package.flags.flags_of_type('build')[2]
    ...
    f = @package.flags.flags_of_type('build')[3]
  ...
  def test_delete_flags_of_type_build__from_project
    # checking precondition
    assert_equal 2, @project.flags.flags_of_type('build').size
  ...
  def test_delete_type_build_flags_from_package
    ...
    # destroy flags
    @package.flags.flags_of_type('build')[0].destroy
    # reload required!
    @package.reload
  ...
  def test_position
    ...
    @project.reload
    assert_equal 3, @project.flags.flags_of_type('build').size
